...
因為不同檔案之間存在依賴關係，所以我們加入了引用標籤(`/// <reference path="" />`)來告訴編譯器檔之間的關聯。
我們的測試程式碼保持不變。
...
編譯器會根據來源程式碼裡的引用標籤自動地對輸出進行排序。你也可以單獨地指定每個檔案。
...
根據編譯時指定的目的模組型別，編譯器會產生相應的程式碼，或者是適合Node.js（commonjs）或者是適合require.js（AMD）模組載入系統的程式碼。
...
就像每個JS檔對應一個模組一樣，TypeScript裡模組檔與產生的JS檔也是一一對應的。
...
我們給identity添加了型別變數‘T’。‘T’幫助我們捕獲使用者傳入的型別（比如：number），之後我們就可以使用這個型別。之後我們再次使用了‘T’當做返回型別。現在我們可以知道參數型別與返回型別是相同的了。
...
Typescript裡宣告可用於三個地方：命名空間/模組，型別或者值。建立命名空間/模組的宣告可以通過（.）識別字訪問其中的型別。建立型別的宣告就是用給定的名字建立相應型別。最後，建立值的宣告是在產生的JavaScript裡存在的那部分（比如：函數和變數）。
