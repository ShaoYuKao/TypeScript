...
為了讓程式有價值，我們需要能夠處理最簡單的資料單元：數字、字串、結構，布林值等。TypeScript支持與JavaScript幾乎相同的資料型別，此外還提供了實用的舉列型別方便我們使用。
...
在對現有JavaScript程式碼進行改寫的時候，`any`型別是十分有用的，它允許你在編譯時可選擇性地包含或移除型別檢查。
...
某種程度上來說，`void`型別與`any`型別是相反的，它表示沒有任何型別。當一個函數沒有返回值時，你通常會見到其返回型別是`void`：
...
TypeScript的核心原則之一是對值所具有的`外形`進行型別檢查。它有時被稱做“鴨子型別”或“結構性子型別”。在TypeScript裡，介面的作用就是為這些型別命名和為你的程式碼或協力廠商程式碼定義契約。
...
型別檢查器會查看`printLabel`的調用。`printLabel`有一個參數，並要求這個物件參數有一個名為`label`型別為`string`的屬性。需要注意的是，我們傳入的物件參數實際上會包含很多屬性，但是編譯器只會檢查那些必需的屬性是否存在，並且其型別是否匹配。

下面我們重寫上面的例子，這次使用介面來描述：必須包含一個`label`屬性且型別為`string`：
...
`LabelledValue`介面就好比一個名字，用來描述上面例子裡的要求。它代表了有一個`label`屬性且型別為`string`的物件。需要注意的是，我們在這裡並不能像在其它語言裡一樣，說傳給`printLabel`的物件實現了這個介面。我們只會去關注值的外形。只要傳入的物件滿足上面提到的必要條件，那麼它就是被允許的。
...
為了使用介面表示函數型別，我們需要給介面定義一個調用簽名。它就像是一個只有參數列表和返回數值型別的函式定義。
...
這樣定義後，我們可以像使用其它介面一樣使用這個函數型別的介面。下例展示了如何創建一個函數型別的變數，並將一個同型別的函數賦值給這個變數。
...
函數的返回數值型別是通過其返回值推斷出來的（此例是`false`和`true`）。
如果讓這個函數返回數位或字串，型別檢查器會警告我們函數的返回數值型別與`SearchFunc`介面中的定義不匹配。

### <a name="2.4"></a>陣列型別

與使用介面描述函數型別差不多，我們也可以描述陣列型別。
陣列型別具有一個`index`型別表示索引的型別，還有一個相應的返回數值型別表示通過索引得到的元素的型別。
...
支援兩種索引型別：string和number。陣列可以同時使用這兩種索引型別，但是有一個限制，數位索引返回值的型別必須是字串索引返回值的型別的子型別。

索引簽名能夠很好的描述陣列和`dictionary`模式，它們也要求所有屬性要與返回數值型別相匹配。下面的例子裡，length屬性與一般的索引返回數值型別不匹配，所以型別檢查器給出一個錯誤提示：
...
當你操作類和介面的時候，你要知道類是具有兩個型別的：靜態部分的型別和實例的型別。你會注意到，當你用構造器簽名去定義一個介面並試圖定義一個類去實現這個介面時會得到一個錯誤：
...
這裡因為當一個類實現了一個介面時，只對其實例部分進行型別檢查。constructor存在於類的靜態部分，所以不在檢查的範圍內。
...
和類一樣，介面也可以相互擴展。擴展介面時會將其它介面裡的屬性拷貝到這個介面裡，因此允許你把介面拆分成單獨的可重用的組件。
...
使用協力廠商庫的時候，你可能會像上面那樣去定義完整的型別。

## <a name="3"></a>類

傳統的JavaScript程式使用函數和基於原型的繼承來創建可重用的元件，但這對於熟悉使用物件導向方式的程式師來說有些棘手，因為他們用的是基於類的繼承並且對象是從類構建出來的。從JavaScript的下個版本ECMAScript6開始，JavaScript程式將可以使用這種基於類的物件導向方法。在TypeScript裡，我們允許開發者現在就使用這些特性，並且編譯後的JavaScript可以在所有主流流覽器和平臺上運行，而不需要等到下個JavaScript版本。
...
它會調用之前定義的構造函數，創建一個`Greeter`型別的新物件，並執行構造函數初始化它。

### <a name="3.2"></a>繼承

在TypeScript裡，我們可以使用常用的物件導向模式。
當然，基於類的程式設計中最基本的模式是允許使用繼承來擴展一個類。
...
你仍然可以使用`public`明確地指定其訪問型別，並且這確實是一個最佳實踐。我們可以用下面的方式來重寫上面的`Animal`類：
...
然而，當我們比較帶有`private`或`protected`成員的型別的時候，情況就不同了。如果其中一個型別裡包含一個`private`成員，那麼只有當另外一個型別中也存在這樣一個`private`成員， 並且它們是來自同一處聲明時，我們才認為這兩個型別是相容的。對於`protected`成員也使用這個規則。
...
還有一個`Employee`類，其型別看上去與`Animal`是相同的。
...
參數屬性通過給構造函數參數添加一個訪問限定詞來聲明。
使用`private`限定一個參數屬性會聲明並初始化一個私有成員；對於`public`和`protected`來說也是一樣。

### <a name="3.4"></a>存取器

TypeScript支援getters/setters來截取對對象成員的訪問。它能説明你有效的控制對物件成員的訪問。
...
當你在TypeScript裡定義類的時候，實際上同時定義了很多東西。首先是類的實例的型別。
...
在這裡，我們寫了`var greeter: Greeter`，意思是`Greeter`類實例的型別是`Greeter`。
...
上面的程式碼裡，‘var Greeter’將被賦值為構造函數。當我們使用‘new’並執行這個函數後，便會得到一個類的實例。這個構造函數也包含了類的所有靜態屬性。換個角度說，我們可以認為類具有實例部分與靜態部分這兩個部分。
...
再之後，我們直接使用類。
我們創建了一個叫做`greeterMaker`的變數。
這個變數保存了這個類或者說保存了類構造函數。
然後我們使用`typeof Greeter`，意思是取Greeter類的型別，而不是實例的型別。
或者理確切的說，"告訴我`Greeter`識別字的型別"，也就是構造函數的型別。
這個型別包含了類的所有靜態成員和構造函數。
之後，就和前面一樣，我們在`greeterMaker`上使用`new`，創建`Greeter`的實例。

#### <a name="3.6.2"></a>把類當做介面使用

如上一節裡所講的，類定義會創建兩個東西：類實例的型別和一個構造函數。因為類可以創建出型別，所以你能夠在可以使用介面的地方使用類。
...
我們也會涉及到一些高級主題，如怎麼使用外部模組，當使用TypeScript模組時如何避免常見的陷井。
...
因為我們想讓這些介面和類在命名空間外也是可訪問的，所以我們需要使用`export`。
相反的，變數`lettersRegexp`和`numberRegexp`是具體實現，所以沒有匯出，因此它們在命名空間外是不能訪問的。
在檔末尾的測試程式碼裡，我們需要限制型別名稱，因為這是在命名空間外訪問，比如`Validation.LettersOnlyValidator`。
...
當應用變得越來越大時，我們需要將程式碼分散到不同的檔中以便於維護。

### <a name="4.1"></a>多檔中的命名空間

現在，我們把`Validation`命名空間分割成多個檔。
儘管是不同的檔，它們仍是同一個命名空間，並且在使用的時候就如同它們在一個檔中定義的一樣。
因為不同檔之間存在依賴關係，所以我們加入了引用標籤來告訴編譯器檔之間的關聯。
我們的測試程式碼保持不變。
...
第一種方式，把所有的輸入檔編譯為一個輸出檔，需要使用`--out`標記：

```Shell
tsc --out sample.js Test.ts
```

編譯器會根據源碼裡的引用標籤自動地對輸出進行排序。你也可以單獨地指定每個檔。

```Shell
tsc --out sample.js Validation.ts LettersOnlyValidator.ts ZipCodeValidator.ts Test.ts
```

第二種方式，我們可以編譯每一個檔（預設方式），那麼每個原始檔案都會對應生成一個JavaScript檔。
然後，在頁面上通過`<script>`標籤把所有生成的JavaScript檔按正確的順序引進來，比如：
...
對於沒有使用Node.js和require.js的應用來說是不需要使用外部模組的，最好使用上面介紹的命名空間的方式來組織程式碼。

使用模組時，不同檔之間的關係是通過檔級別的導入和匯出來指定的。
在TypeScript裡，任何具有頂級`import`和`export`的檔都會被視為模組。

下面，我們把之前的例子改寫成使用模組。
注意，我們不再使用`module`關鍵字 - 檔本身會被視為一個模組並以檔案名來區分。

引用標籤用`import`語句來代替，指明了模組之前的依賴關係。
`import`語句有兩部分：模組在當前檔中的名字，`require`關鍵字指定了依賴模組的路徑：

```typescript
import someMod = require('someModule');
```

我們通過頂級的`export`關鍵字指出了哪些物件在模組外是可見的，如同使用`export`定義命名空間的公共介面一樣。

為了編譯，我們必須在命令列上指明生成模組的目標型別。對於Node.js，使用*--module commonjs*。對於require.js，使用`--module amd`。比如：

```Shell
ts --module commonjs Test.ts
```

編譯的時候，每個外部模組會變成一個單獨的檔。如同引用標籤，編譯器會按照*import*語句編譯相應的檔。
...
根據編譯時指定的目的模組型別，編譯器會生成相應的程式碼，或者是適合Node.js（commonjs）或者是適合require.js（AMD）模組載入系統的程式碼。
想要瞭解更多關於`define`和`require`函數的使用方法，請閱讀相應模組載入器的說明文檔。

這個例子展示了在導入匯出階段使用的名字是怎麼轉換成模組載入程式碼的。
...
像這種情況，在驗證器物件前面再加上限定名就顯得累贅了，最好是直接使用一個識別字。

`export =`語法指定了模組匯出的單個物件。
它可以是類，介面，模組，函數或列舉型別。
當import的時候，直接使用模組匯出的識別字，不再需要其它限定名。
...
不要與`import x = require('name')`用來載入模組的語法弄混了，這裡的語法是為指定的符號創建一個別名。
你可以用這種方法為任意識別字創建別名，也包括導入的模組中的物件。
...
重要的是，對於值來講，`import`會產生與原始符號不同的引用，所以改變別名的值並不會影響原始變數的值。

### <a name="4.5"></a>可選模組的載入與其它高級載入的場景

有些時候，你只想在某種條件下才去載入一個模組。
在TypeScript裡，我們可以使用下面的方式來實現它以及其它高級載入的場景，直接調用模組載入器而不必擔心型別安全問題。

編譯器能探測出一個模組是否在生成的JavaScript裡被使用到了。
對於那些只做為型別系統部分使用的模組來講，不會生成對應`require程式碼`。
挑出未使用的引用有益於性能優化，同時也允許可選擇性的載入模組。

這種模式的核心是`import id = require('...')`讓我們可以訪問外部模組匯出的型別。
模組載入是動態調用的（通過`require`），像下面`if`語句展示的那樣。
它利用了挑出對未使用引用的優化，模組只在需要的時候才去載入。
為了讓這種方法可行，通過`import`定義的符號只能在表示型別的位置使用（也就是說那段程式碼永遠不會被編譯生成JavaScript）。

為了確保使用正確，我們可以使用`typeof`關鍵字。
在要求是型別的位置使用`typeof`關鍵字時，會得到型別值，在這個例子裡得到的是外部模組的型別。
...
為了描述不是用TypeScript寫的程式庫的型別，我們需要對程式庫暴露的API進行聲明。
由於大部分程式庫只提供少數的頂級物件，命名空間和模組是用來表示它們是一個好辦法。
我們叫它聲明不是對執行環境的定義。
通常會在`.d.ts`裡寫這些定義。
...
流行的程式庫D3在全域物件`d3`裡定義它的功能。
因為這個庫通過一個`<script>`標籤載入（不是通過模組載入器），它的聲明檔使用內部模組來定義它的型別。
為了讓TypeScript編譯器識別它的型別，我們使用外部命名空間聲明。
比如，我們像下面這樣寫：
...
我們可以使用頂級export聲明來為每個模組定義各自的`.d.ts`檔，但全部放在一個大的檔中會更方便。
...
### <a name="4.7"></a>命名空間和模組的陷井

這一節，將會介紹使用內部和外部模組時常見的陷井和怎麼去避免它。

#### <a name="4.7.1"></a>對模組使用`/// <reference>`

一個常見的錯誤是使用`/// <reference>`引用模組檔，應該使用import。
要理解這之間的不同，我們首先應該弄清編譯器是怎麼找到模組的型別資訊的。

首先，根據`import x = require(...);`聲明查找`.ts`文件。
這個檔應該是使用了頂級import或export聲明的執行檔。

其次，與前一步相似，去查找`.d.ts`檔，不同的是它不是執行檔而是聲明檔（同樣具有頂級的import或export聲明）。
...
模組檔本身已經是一個邏輯分組，並且它的名字是由導入這個模組的程式碼指定，所以沒有必要為匯出的物件增加額外的模組層。
...
就像每個JS檔對應一個模組一樣，TypeScript裡模組檔與生成的JS檔也是一一對應的。
這會產生一個效果，就是無法使用`--out`來讓編譯器合併多個模組檔為一個JavaScript檔。
...
我們可以給每個參數添加型別之後再為函數本身添加返回數值型別。TypeScript能夠根據返回語句自動推斷出返回數值型別，因此我們通常省略它。
...
函數型別包含兩部分：參數型別和返回數值型別。當寫出完整函數型別的時候，這兩部分都是需要的。我們以參數列表的形式寫出參數型別，為每個參數指定一個名字和型別。這個名字只是為了增加可讀性。我們也可以這麼寫：
...
對於返回值，我們在函數和返回數值型別之前使用(=>)符號，使之清晰明瞭。如之前提到的，返回數值型別是函數型別的必要部分，如果函數沒有返回任何值，你也必須指定返回數值型別為‘void’而不能留空。

函數的型別只是由參數型別和返回值組成的。函數中使用的‘捕獲’變數不會體現在型別裡。實際上，這些變數是函數的隱藏狀態並不是組成API的一部分。
...
TypeScript裡，我們還可以為可選參數設置預設值。仍然修改上例，把last name的預設值設置為“Smith”。
...
共用同樣的型別`(firstName: string, lastName?: string) => string`。默認參數的預設值消失了，只保留了它是一個可選參數的資訊。
...
JavaScript裡，‘this’的值在函數被調用的時候才會指定。這是個既強大又靈活的特點，但是你需要花點時間弄清楚函式呼叫的上下文是什麼。眾所周知這不是一件很簡單的事，特別是函數當做回呼函數使用的時候。
...
當你調用`cardPicker()`時會發生這種情況。這裡沒有對`this`進行動態繫結因此為window。（注意在嚴格模式下，會是undefined而不是window）。

為了解決這個問題，我們可以在函數被返回時就綁好正確的`this`。
這樣的話，無論之後怎麼使用它，都會引用綁定的‘deck’對象。

我們把函數運算式變為使用lambda運算式（ () => {} ）。這樣就會在函數創建的時候就指定了‘this’值，而不是在函式呼叫的時候。
...
‘pickCard’方法根據傳入參數的不同會返回兩種不同的型別。如果傳入的是代表紙牌的物件，函數作用是從中抓一張牌。如果用戶想抓牌，我們告訴他抓到了什麼牌。但是這怎麼在型別系統裡表示呢。

方法是為同一個函數提供多個函數型別定義來進行函數重載。編譯器會根據這個清單去處理函數的調用。下面我們來重載‘pickCard’函數。
...
這樣改變後，重載的`pickCard`函數在調用的時候會進行正確的型別檢查。
...
軟體工程中，我們不僅要創建一致的定義良好的API，同時也要考慮再使用性。元件不僅能夠支援當前的資料型別，同時也能支援未來的資料型別，這在創建大型系統時為你提供了十分靈活的功能。

在像C#和Java這樣的語言中，可以使用‘泛型’來創建可重用的元件，一個元件可以支援多種型別的資料。這樣使用者就可以以自己的資料型別來使用元件。
...
雖然使用`any`型別後這個函數已經能接收任何型別的arg參數，但是卻丟失了一些資訊：傳入的型別與返回的型別應該是相同的。
如果我們傳入一個數字，我們只知道任何型別的值都有可能被返回。

因此，我們需要一種方法使用返回值的型別與傳入參數的型別是相同的。
...
我們給identity添加了型別變數‘T’。‘T’幫助我們捕獲用戶傳入的型別（比如：number），之後我們就可以使用這個型別。之後我們再次使用了‘T’當做返回數值型別。現在我們可以知道參數型別與返回數值型別是相同的了。

我們把這個版本的identity函數叫做泛型，它可以適用於多個型別。不像使用‘any’，它不會丟失資訊。同時也可以像第一個例子那像，傳入數數值型別並返回數數值型別。
...
使用泛型創建像`identity`這樣的泛型函數時，編譯器要求你在函數體必須正確的使用這個通用的型別。換句話說，你必須把這些參數當做是任意或所有型別。
...
使用過其它語言的話，你可能對這種語法已經很熟悉了。在下一節，會介紹如何創建自訂泛型像Array<T>一樣。

### <a name="6.3"></a>泛型型別

上一節，我們創建了identity通用函數，可以適用於不同的型別。在這節，我們研究一下函數本身的型別，以及如何創建泛型介面。
...
我們並沒有描述泛型函數，而是使用一個非泛型函數簽名作為泛型型別一部分。當我們使用GenericIdentityFn的時候，我也得傳入一個型別參數來指定泛型型別（這個例子是：number），鎖定了之後程式碼裡使用的型別。
...
‘GenericNumber’類的使用是十分直觀的，並且你應該注意到了我們並不限制只能使用數字型別。也可以使用字串或其它更複雜的型別。
...
我們在[類](#類)那節說過，類有兩部分：靜態部分和實例部分。泛型類指的是實例部分的型別，所以類的靜態屬性不能使用這個泛型型別。
...
為此，我們定義一個介面來描述約束條件。創建一個包含‘length’屬性的介面，使用這個介面和extends關鍵字還實現約束。
...
注意：上面兩種寫法並不完全等同，因為第一段程式的返回值可能是U，而第二段程式卻沒有這一限制。

#### <a name="6.5.2"></a>在泛型裡使用類型別

在TypeScript使用泛型創建工廠函數時，需要引用構造函數的類型別。比如，
...
檢查檔編碼是不是UTF-8 - [https://typescript.codeplex.com/workitem/1587](https://typescript.codeplex.com/workitem/1587)
...
除了傳統的物件導向繼承方式，還有一種流行的從可重用元件中創建類的方式，就是通過聯合一個簡單類的程式碼。你可能在Scala這樣的語言裡對mixins已經熟悉了，它在JavaScript中也是很流行的。
...
我們可以這麼做來達到目的，為將要mixin進來的屬性方法創建出占位元屬性。這告訴編譯器這些成員在運行時是可用的。這樣就能使用mixin帶來的便利，雖說需要提前定義一些占位元屬性。
...
最後，創建這個説明函數，幫我們做混入操作。它會遍歷mixins上的所有屬性，並複製到目標上去，把之前的占位元屬性替換成真正的實現程式碼。
...
TypeScript有一些獨特的概念，有的是因為我們需要描述JavaScript頂級物件的型別發生了什麼變化。這其中之一叫做‘聲明合併’。理解了這個概念對於你使用TypeScript去操作現有的JavaScript是大有幫助的。同時，也開啟通往更多高級抽象概念的大門。

首先，在瞭解怎麼進行聲明合併之前，讓我們先看一下什麼叫做‘聲明合併’。

在這個手冊裡，聲明合併是指編譯器會把兩個相同的名字的聲明合併成一個單獨的聲明。合併後的聲明同時具有那兩個被合併的聲明的特性。聲明合併不限於只合併兩個，任意數量都可以。

### <a name="9.1"></a>基礎概念

Typescript裡聲明可用於三個地方：命名空間/模組，型別或者值。創建命名空間/模組的聲明可以通過（.）識別字訪問其中的型別。創建型別的聲明就是用給定的名字創建相應型別。最後，創建值的聲明是在生成的JavaScript裡存在的那部分（比如：函數和變數）。
...
最簡單最常見的就是合併介面，聲明合併的種類是：介面合併。從根本上說，合併的機制是把各自聲明裡的成員放進一個同名的單一介面裡。
...
介面中非函數的成員必須是唯一的。如果多個介面中具有相同名字的非函數成員就會報錯。
...
這三個介面合併成一個聲明。注意每組介面裡的聲明順序保持不變，只是靠後的介面會出現在它前面的介面聲明之前。
...
與介面相似，同名的模組也會合並其成員。模組會創建出命名空間和值，我們需要知道這兩者都是怎麼合併的。

命名空間的合併，模組匯出的同名介面進行合併，構成單一命名空間內含合併後的介面。

值的合併，如果當前已經存在給定名字的模組，那麼後來的模組的匯出成員會被加到已經存在的那個模組裡。
...
模組可以與其它型別的聲明進行合併。只要模組的定義符合將要合併型別的定義。合併結果包含兩者的聲明型別。Typescript使用這個功能去實現一些JavaScript裡的設計模式。
...
除了內部類的模式，你在JavaScript裡，創建一個函數稍後擴展它增加一些屬性也是很常見的。Typescript使用聲明合併來達到這個目的並保證型別安全。
...
為了推斷x的型別，我們必須考慮所有元素的型別。這裡有兩種選擇：數字和null。計算通用型別演算法會考慮所有的候選型別，並給出一個相容所有候選型別的型別。

由於最終的通用型別取自候選型別，有些時候候選型別共用相同的通用型別，但是卻沒有一個型別能做為所有候選型別的型別。例如：
...
如果沒有找到最佳通用型別的話，型別推論的結果是空對象型別，`{}`。
因為這個型別沒有任何成員，所以訪問其成員的時候會報錯。

### <a name="10.3"></a>上下文型別

TypeScript型別推論也可能按照相反的方向進行。
這被叫做“按上下文歸類”。按上下文歸類會發生在運算式的型別與所處的位置相關時。比如：
...
如果上下文型別運算式包含了明確的型別資訊，上下文的型別被忽略。
重寫上面的例子：
...
這個函數運算式有明確的參數型別注解，上下文型別被忽略。
這樣的話就不報錯了，因為這裡不會使用到上下文型別。

上下文歸類會在很多情況下使用到。
通常包含函數的參數，賦值運算式的右邊，型別斷言，物件成員和陣列字面量和返回值語句。
...
TypeScript裡的型別相容性是以結構性子型別來判斷的。結構性型別是完全根據成員關聯型別的一種方式。與正常的型別判斷不同。看下面的例子：
...
在正常使用型別的語言像C#或Java中，這段程式碼會報錯，因為Person類沒有明確說明其實現了Named介面。

TypeScript的結構性子型別是根據JavaScript程式碼的通常寫法來設計的。因為JavaScript裡常用匿名物件像函數運算式或物件字面量，所以用結構性型別系統來描述這些型別比使用正常的型別系統更好。

### <a name="11.0"></a>關於穩定性的注意事項

TypeScript的型別系統允許那些在編譯階段無法否認其安全性的操作。當一個型別系統具有此屬性時，被當做是“不穩定”的。TypeScript裡允許這種不穩定行為發生的地方是經過仔細考慮的。通過這篇文章，我們會解釋什麼時候會發生這種情況和其背景。
...
這裡要檢查y是否能賦值給x，編譯器x中的每個屬性，看是否能在y中也找到對應屬性。在這個例子中，y必須包含名字是‘name’的string型別成員。y滿足條件，因此賦值正確。
...
要查看x是否能賦值給y，首先看它們的參數列表。x的每個參數必須能在y裡找到對應型別的參數。注意的是參數的名字相同與否無所謂，只看它們的型別。這裡，x的每個參數在y中都能找到對應的參數，所以允許賦值。

第二個賦值錯誤，因為y有個必需的第二個參數，但是x並沒有，所以不允許賦值。

你可能會疑惑為什麼允許‘忽略’參數，像例子y=x中那樣。原因是忽略額外的參數在JavaScript裡是很常見的。例如，Array#forEach給回呼函數傳3個參數：陣列元素，索引和整個陣列。儘管如此，傳入一個隻使用第一個參數的回呼函數也是很有用的：
...
下面來看看如何處理返回數值型別，創建兩個僅是返回數值型別不同的函數：
...
型別系統強制源函數的返回數值型別必須是目標函數返回數值型別的子型別。

#### <a name="11.2.1"></a>函數參數雙向協變

當比較函數參數型別時，只有源函數參數能夠賦值給目標函數或反過來才匹配成功。這是不穩定的，因為調用者可能會被給予一個函數，它接受一個更確切型別，但是調用函數使用不那麼確切的型別。實際上，這極少會發生錯誤，並且能夠實現很多JavaScript裡的常見模式。例如：
...
這對於型別系統來說是不穩定的，但從運行時的角度來看，可選參數一般來說是不強制的，因為對於大多數函數來說相當於傳遞了一些‘undefinded’。

有一個好的例子，常見的函數接收一個回呼函數並用對於程式師來說是可預知的參數但對型別系統來說是不確定的參數來調用：
...
對於有重載的函數，源函數的每個重載都要在目標函數上找到對應的函數簽名。這確保了目標函數可以在所有源函數可調用的地方調用。對於特殊的函數重載簽名不會用來做相容性檢查。
...
類與物件字面量和介面差不多，但有一點不同：類有靜態部分和實例部分的型別。比較兩個類型別的物件時，只有實例的成員會被比較。靜態成員和構造函數不在比較的範圍內。
...
私有成員會影響相容性判斷。當類的實例用來檢查相容時，如果它包含一個私有成員，那麼目標型別必須包含來自同一個類的這個私有成員。這允許子類賦值給父類，但是不能賦值給其它有同樣型別的類。
...
它們的不同點在於，賦值擴展了子型別相容，允許給`any`賦值或從`any`取值和允許數位賦值給列舉型別或列舉型別賦值給數位。
...
當使用外部JavaScript庫或新的宿主API時，你需要一個聲明檔（.d.ts）定義程式庫的shape。這個手冊包含了寫.d.ts檔的高級概念，並帶有一些例子，告訴你怎麼去寫一個聲明檔。

### <a name="12.1"></a>指導與說明

#### <a name="12.1.1"></a>流程

最好從程式庫的文檔開始寫.d.ts檔，而不是程式碼。這樣保證不會被具體實現所干擾，而且相比於JS程式碼更易讀。下面的例子會假設你正在參照文檔寫聲明檔。

#### <a name="12.1.2"></a>命名空間

當定義介面（例如：“options”物件），你會選擇是否將這些型別放進命名空間裡。
這主要是靠主觀判斷 -- 使用的人主要是用這些型別宣告變數和參數，並且型別命名不會引起命名衝突，放在全域命名空間裡更好。
如果型別不是被直接使用，或者沒法起一個唯一的名字的話，就使用命名空間來避免與其它型別發生衝突。

#### <a name="12.1.3"></a>回呼函數

許多JavaScript庫接收一個函數做為參數，之後傳入已知的參數來調用它。當為這些型別與函數簽名的時候，不要把這個參數標記成可選參數。正確的思考方式是“會提供什麼樣的參數？”，不是“會使用到什麼樣的參數？”。TypeScript 0.9.7+不會強制這種可選參數的使用，參數可選的雙向協變可以被外部的linter強制執行。

#### <a name="12.1.4"></a>擴展與聲明合併

寫聲明檔的時候，要記住TypeScript擴展現有物件的方式。你可以選擇用匿名型別或介面類別型的方式聲明一個變數：
...
從使用者角度來講，它們是相同的，但是SomePoint型別能夠通過介面合併來擴展：
...
TypeScript的類會創建出兩個型別：實例型別，定義了型別的實例具有哪些成員；構造函數型別，定義了類構造函數具有哪些型別。構造函數型別也被稱做類的靜態部分型別，因為它包含了類的靜態成員。

你可以使用typeof關鍵字來拿到類靜態部分型別，在寫聲明檔時，想要把類明確的分解成實例型別和靜態型別時是有用且必要的。
...
下面進行例子部分。對於每個例子，先是使用使用方法，然後是型別宣告。如果有多個好的聲明表示方法，會列出多個。

#### <a name="12.2。1"></a>參數對象
