...
TypeScript的核心原則之一是對值所具有的`形體`進行型別檢查。它有時被稱做“鴨子型別”(duck typing)或“結構性子型別”(structural subtyping)。在TypeScript裡，介面的作用就是為這些型別命名和為你的程式碼或協力廠商程式碼定義契約。
...
型別檢查器會查看`printLabel`的呼叫。`printLabel`有一個參數，並且要求這個物件參數有一個名為`label`型別為`string`的屬性。需要注意的是，我們傳入的物件參數實際上會包含很多屬性，但是編譯器只會檢查那些必需的屬性是否存在，並且其型別是否符合。
...
為了使用介面表示函數型別，我們需要給介面定義一個呼叫簽章。它就像是一個只有參數列表和返回型別的函式定義。
...
函數的返回型別是通過其返回值推斷出來的（此例是`false`和`true`）。
如果讓這個函數返回`number`或`string`，型別檢查器會警告我們函數的返回型別與`SearchFunc`介面中的定義不相符。

### <a name="2.4"></a>陣列型別

與使用介面描述函數型別差不多，我們也可以描述陣列型別。
陣列型別具有一個`index`型別表示索引的型別，還有一個相應的返回型別表示通過索引得到的元素的型別。
...
支援兩種索引型別：`string`和`number`。陣列可以同時使用這兩種索引型別，但是有一個限制，數字索引返回值的型別必須是字串索引返回值的型別的子型別。

索引簽名能夠很好的描述陣列和`dictionary`模式，它們也要求所有屬性要與返回型別相符合。下面的例子裡，length屬性與一般的索引返回型別不符合，所以型別檢查器給出一個錯誤提示：
...
最後一行，我們使用`new`構造了Greeter類的一個執行個體。
...
注意，我們不能在`Person`類外使用`name`，但是我們仍然可以通過`Employee`類的執行個體方法訪問，因為`Employee`是由`Person`派生出來的。
...
到目前為止，我們只討論了類的執行個體成員，那些僅當類被產生實體的時候才會被初始化的屬性。
我們也可以創建類的靜態成員，這些屬性存在於類本身上面而不是類的執行個體上。
在這個例子裡，我們使用`static`定義`origin`，因為它是所有網格都會用到的屬性。
每個執行個體想要訪問這個屬性的時候，都要在origin前面加上類名。
...
當你在TypeScript裡定義類的時候，實際上同時定義了很多東西。首先是類的執行個體的型別。
...
上面的程式碼裡，‘var Greeter’將被賦值為構造函數。當我們使用‘new’並執行這個函數後，便會得到一個類的執行個體。這個構造函數也包含了類的所有靜態屬性。換個角度說，我們可以認為類具有執行個體部分與靜態部分這兩個部分。
...
然後我們使用`typeof Greeter`，意思是取Greeter類的型別，而不是執行個體的型別。
或者理確切的說，"告訴我`Greeter`識別字的型別"，也就是構造函數的型別。
這個型別包含了類的所有靜態成員和構造函數。
之後，就和前面一樣，我們在`greeterMaker`上使用`new`，創建`Greeter`的執行個體。

#### <a name="3.6.2"></a>把類當做介面使用

如上一節裡所講的，類定義會創建兩個東西：類執行個體的型別和一個構造函數。因為類可以創建出型別，所以你能夠在可以使用介面的地方使用類。
...
### <a name="4.5"></a>選擇性模組的載入與其它高級載入的場景
...
挑出未使用的引用有益於性能優化，同時也允許選擇性擇性的載入模組。
...
最後，在`declare`的模組裡尋找名字符合的“外來模組的聲明”。
...
函數型別包含兩部分：參數型別和返回型別。當寫出完整函數型別的時候，這兩部分都是需要的。我們以參數列表的形式寫出參數型別，為每個參數指定一個名字和型別。這個名字只是為了增加可讀性。我們也可以這麼寫：
...
共用同樣的型別`(firstName: string, lastName?: string) => string`。默認參數的預設值消失了，只保留了它是一個選擇性參數的資訊。

### <a name="5.4"></a>剩餘參數

必要參數，默認參數和選擇性參數有個共同點：它們表示某一個參數。有時，你想同時操作多個參數，或者你並不知道會有多少參數傳遞進來。在JavaScript裡，你可以使用arguments來訪問所有傳入的參數。
...
剩餘參數會被當做個數不限的選擇性參數。可以一個都沒有，同樣也可以有任意個。編譯器創建參數陣列，名字是你在省略號（...）後面給定的名字，你可以在函數體內使用這個陣列。
...
我們給identity添加了型別變數‘T’。‘T’幫助我們捕獲用戶傳入的型別（比如：number），之後我們就可以使用這個型別。之後我們再次使用了‘T’當做返回型別。現在我們可以知道參數型別與返回型別是相同的了。
...
我們在[類](#類)那節說過，類有兩部分：靜態部分和執行個體部分。泛型類指的是執行個體部分的型別，所以類的靜態屬性不能使用這個泛型型別。
...
一個更高級的例子，使用原型屬性推斷並約束構造函數與類執行個體的關係。
...
型別系統強制源函數的返回型別必須是目標函數返回型別的子型別。

#### <a name="11.2.1"></a>函數參數雙向協變

當比較函數參數型別時，只有源函數參數能夠賦值給目標函數或反過來才符合成功。這是不穩定的，因為調用者可能會被給予一個函數，它接受一個更確切型別，但是調用函數使用不那麼確切的型別。實際上，這極少會發生錯誤，並且能夠實現很多JavaScript裡的常見模式。例如：
...
類與物件字面量和介面差不多，但有一點不同：類有靜態部分和執行個體部分的型別。比較兩個類型別的物件時，只有執行個體的成員會被比較。靜態成員和構造函數不在比較的範圍內。
...
私有成員會影響相容性判斷。當類的執行個體用來檢查相容時，如果它包含一個私有成員，那麼目標型別必須包含來自同一個類的這個私有成員。這允許子類賦值給父類，但是不能賦值給其它有同樣型別的類。
...
許多JavaScript庫接收一個函數做為參數，之後傳入已知的參數來調用它。當為這些型別與函數簽名的時候，不要把這個參數標記成選擇性參數。正確的思考方式是“會提供什麼樣的參數？”，不是“會使用到什麼樣的參數？”。TypeScript 0.9.7+不會強制這種選擇性參數的使用，參數選擇性的雙向協變可以被外部的linter強制執行。
...
TypeScript的類會創建出兩個型別：執行個體型別，定義了型別的執行個體具有哪些成員；構造函數型別，定義了類構造函數具有哪些型別。構造函數型別也被稱做類的靜態部分型別，因為它包含了類的靜態成員。

你可以使用typeof關鍵字來拿到類靜態部分型別，在寫聲明檔時，想要把類明確的分解成執行個體型別和靜態型別時是有用且必要的。
