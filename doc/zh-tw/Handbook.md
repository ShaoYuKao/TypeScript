...
你會注意到，我們在引用任何一個類別成員的時候都用了`this`。
它表示我們訪問的是類別的成員。
...
在TypeScript裡，我們可以使用下面的方式來實現它以及其它高級載入的場景，直接呼叫模組載入器而不必擔心型別安全問題。
...
為了描述不是用TypeScript寫的程式庫的型別，我們需要對程式庫暴露的API進行宣告。
...
因為這個庫通過一個`<script>`標籤載入（不是通過模組載入器），它的宣告檔使用內部模組來定義它的型別。
...
我們可以使用頂級export宣告來為每個模組定義各自的`.d.ts`檔，但全部放在一個大的檔中會更方便。
...
首先，根據`import x = require(...);`宣告查找`.ts`文件。
這個檔應該是使用了頂級import或export宣告的執行檔。

其次，與前一步相似，去查找`.d.ts`檔，不同的是它不是執行檔而是宣告檔（同樣具有頂級的import或export宣告）。

最後，在`declare`的模組裡尋找名字符合的“外來模組的宣告”。
...
剩餘參數會被當做個數不限的選擇性參數。可以一個都沒有，同樣也可以有任意個。編譯器建立參數陣列，名字是你在省略號（...）後面給定的名字，你可以在函數體內使用這個陣列。
...
軟體工程中，我們不僅要建立一致的定義良好的API，同時也要考慮再使用性。元件不僅能夠支援當前的資料型別，同時也能支援未來的資料型別，這在建立大型系統時為你提供了十分靈活的功能。

在像C#和Java這樣的語言中，可以使用‘泛型’來建立可重用的元件，一個元件可以支援多種型別的資料。這樣使用者就可以以自己的資料型別來使用元件。

### <a name="6.1"></a>泛型之Hello World

下面來建立第一個使用泛型的例子：identity函數。這個函數會返回任何傳入它的值。你可以把這個函數當成是‘echo’命令。
...
使用泛型建立像`identity`這樣的泛型函數時，編譯器要求你在函數體必須正確的使用這個通用的型別。換句話說，你必須把這些參數當做是任意或所有型別。
...
上一節，我們建立了identity通用函數，可以適用於不同的型別。在這節，我們研究一下函數本身的型別，以及如何建立泛型介面。
...
為此，我們定義一個介面來描述約束條件。建立一個包含‘length’屬性的介面，使用這個介面和extends關鍵字還實現約束。
...
在TypeScript使用泛型建立工廠函數時，需要引用建構函式的類型別。比如，
...
一個更高級的例子，使用原型屬性推斷並約束建構函式與類執行個體的關係。
...
TypeScript有一些獨特的概念，有的是因為我們需要描述JavaScript頂級物件的型別發生了什麼變化。這其中之一叫做‘宣告合併’。理解了這個概念對於你使用TypeScript去操作現有的JavaScript是大有幫助的。同時，也開啟通往更多高級抽象概念的大門。

首先，在瞭解怎麼進行宣告合併之前，讓我們先看一下什麼叫做‘宣告合併’。

在這個手冊裡，宣告合併是指編譯器會把兩個相同的名字的宣告合併成一個單獨的宣告。合併後的宣告同時具有那兩個被合併的宣告的特性。宣告合併不限於只合併兩個，任意數量都可以。

### <a name="9.1"></a>基礎概念

Typescript裡宣告可用於三個地方：命名空間/模組，型別或者值。建立命名空間/模組的宣告可以通過（.）識別字訪問其中的型別。建立型別的宣告就是用給定的名字建立相應型別。最後，建立值的宣告是在生成的JavaScript裡存在的那部分（比如：函數和變數）。
...
當比較函數參數型別時，只有源函數參數能夠賦值給目標函數或反過來才符合成功。這是不穩定的，因為呼叫者可能會被給予一個函數，它接受一個更確切型別，但是呼叫函數使用不那麼確切的型別。實際上，這極少會發生錯誤，並且能夠實現很多JavaScript裡的常見模式。例如：
...
類與物件字面量和介面差不多，但有一點不同：類有靜態部分和執行個體部分的型別。比較兩個類型別的物件時，只有執行個體的成員會被比較。靜態成員和建構函式不在比較的範圍內。
...
當使用外部JavaScript庫或新的宿主API時，你需要一個宣告檔（.d.ts）定義程式庫的shape。這個手冊包含了寫.d.ts檔的高級概念，並帶有一些例子，告訴你怎麼去寫一個宣告檔。

### <a name="12.1"></a>指導與說明

#### <a name="12.1.1"></a>流程

最好從程式庫的文檔開始寫.d.ts檔，而不是程式碼。這樣保證不會被具體實現所干擾，而且相比於JS程式碼更易讀。下面的例子會假設你正在參照文檔寫宣告檔。
...
許多JavaScript庫接收一個函數做為參數，之後傳入已知的參數來呼叫它。當為這些型別與函數簽名的時候，不要把這個參數標記成選擇性參數。正確的思考方式是“會提供什麼樣的參數？”，不是“會使用到什麼樣的參數？”。TypeScript 0.9.7+不會強制這種選擇性參數的使用，參數選擇性的雙向協變可以被外部的linter強制執行。
