...
注意，我們不能在`Person`類外使用`name`，但是我們仍然可以通過`Employee`類的例項方法訪問，因為`Employee`是由`Person`派生出來的。
...
到目前為止，我們只討論了類的例項成員，那些僅當類被例項化的時候才會被初始化的屬性。
