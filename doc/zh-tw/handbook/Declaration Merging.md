...
首先，在了解如何进行声明合并之前，让我们先看一下什幺叫做`声明合并`。
...
理解每个声明创建了什幺，有助于理解当声明合并时什幺东西被合并了。

理解了每种声明会对应创建什幺对于理解如果进行声明合并是有帮助的。
...
命名空间会创建出命名空间和值，我们需要知道这两者都是怎幺合并的。

命名空间的合并，模块导出的同名接口进行合并，构成单一命名空间内含合并后的接口。

值的合并，如果当前已经存在给定名字的命名空间，那幺后来的命名空间的导出成员会被加到已经存在的那个模块里。
